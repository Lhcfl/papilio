...
export const MkMentionsList = (props: { visibility?: 'specified' }) => {
  ...
  const { data, isLoading, isFetchingNextPage, isError, fetchNextPage, hasNextPage } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => api.request('notes/mentions', { untilId: pageParam, limit: 30, visibility }).then(ns => registerNote(...ns)),
  ...
  return (
    <div className="mk-mentions w-full">
      ...
      {data && data.pages.flat().map(m => (
        <MkNote key={m} noteId={m} />
      ))}
      {(isLoading || isFetchingNextPage) && <div className="w-full p-3 flex justify-center"><Spinner /></div>}
