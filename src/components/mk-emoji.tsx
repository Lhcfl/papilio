...
export const MkCustomEmoji = (props: {
	...
	const className =
		"inline h-[2em] align-middle transition-all" +
		(props.normal ? " h-[1.25m] align-[-0.25em]" : "");
	const { emojisMap, site, meta } = useMisskeyGlobal();
	const [error, setError] = useState(false);

	const name = props.name[0] === ":" ? props.name.slice(1, -1) : props.name;
	const isLocal =
		props.host == null && (name.endsWith("@.") || name.includes("@"));
	const url = props.url
		? props.url
		: isLocal
...
export const MkEmoji = (props: {
	...
	return (
		<span className="mk-emoji" dangerouslySetInnerHTML={{ __html: parsed }} />
	);
};
