...
export const MkImage = (props: { image: DriveFile } & HTMLProps<HTMLDivElement>) => {
  ...
  return (
    <div
      className={clsx('mk-image relative w-full flex items-center justify-center bg-muted rounded-md overflow-hidden', className)}
      {...rest}
    >
      <Image src={url} alt={image.name || ''} loading="lazy" decoding="async" onLoad={() => setLoading(false)} onError={() => setLoading(false)} />
      {loading
        && (
