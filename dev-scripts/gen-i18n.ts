/*
 * SPDX-FileCopyrightText: sharkey-project and misskey-project
...
const clean = (text: string) =>
  text.replace(new RegExp(String.fromCodePoint(0x08), 'g'), '').replaceAll(new RegExp(/\\+\{/, 'g'), '{');
...
function readLocale(locale: string) {
  return Object.fromEntries(
    languages.map((lang) => [
      lang,
      yaml.load(clean(tryReadFile(new URL(`locales-source/${locale}/${lang}.yml`, PROJECT_ROOT), 'utf-8'))) || {},
    ]),
  );
}
